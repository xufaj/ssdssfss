...
client.on('message', async message => {
  ...
  if(message.content.startsWith(prefix + "mute")) {
    ...
    let mention = message.mentions.members.first();//حقوق SAJED
    if(!mention) return  message.channel.send('').then(msg => { //حقوق  SAJED
      msg.delete(3500);
      message.delete(3500);
    });
    ...
    if(mention.id === message.author.id) return message.channel.send('**:x:You Cannot give mute to your self**').then(msg => {
      msg.delete(3500);
      message.delete(3500); //حقوق SAJED
    });
   
    if(mention.hasPermission('ADMINISTRATOR')) return message.channel.send(`**:x: لا يمكن آعطاء ميوت لادارة السيرفر**`); //حقوقSAJED
    ...
    .addField('**__السيرفر__**',[ message.guild.name ]) //حقوق  SAJED
    ...
    if(!role) try {
      message.guild.createRole({
        name: "Muted",
        permissions: 0 //حقوق SAJED
      }).then(r => {
        message.guild.channels.forEach(c => {
          c.overwritePermissions(r , {
            SEND_MESSAGES: false, //حقوق SAJED
    ...
    mention.addRole(role).then(() => {
      ...
      mention.setMute(true); //حقوق  SAJED
    });
    setTimeout(() => {
      if(duration === 0) return;
      mention.setMute(false);
      mention.removeRole(role)
    },duration * 60000); //حقوق  SAJED
  }
});
client.on('message', async message => {
...
let command = message.content.split(" ")[0];
     command = command.slice(prefix.length);
    let args = message.content.split(" ").slice(1);  //حقوق  SAJED
if(command === `unmute`) {2
...
client.on("message", (message) => {
    ///حقوق SAJED
   if (message.content.startsWith("$new")) {     /// حقوق  SAJED
        const reason = message.content.split(" ").slice(1).join(" ");     /// حقوق SAJED
...
client.user.setGame(`$help | DREAM D System`,"http://twitch.tv/DJ")
